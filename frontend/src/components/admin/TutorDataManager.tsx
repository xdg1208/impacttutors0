...
import { 
  ...
  MessageCircle, Star, Activity, Clock, CheckCircle, AlertCircle
} from "lucide-react";
              ...
              <button 
                ...
                onClick={async () => {
                   if (confirm(`Are you sure you want to ${selectedTutor.is_onboarded ? 'deactivate' : 'activate'} this tutor?`)) {
                      ...
                      setStatus(null);
                      try {
                        const result = await toggleOnboardingStatus(selectedTutor.id, !selectedTutor.is_onboarded);
                        if (result.success) {
                           setStatus({ type: 'success', message: `Tutor ${selectedTutor.is_onboarded ? 'deactivated' : 'activated'} successfully.` });
                           setTimeout(() => handleClose(), 1500);
                        } else {
                           setStatus({ type: 'error', message: result.error || "Failed to update tutor." });
                        }
                      } catch (err) {
                ...
                className={`flex-1 py-2.5 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2 disabled:opacity-50 ${selectedTutor.is_onboarded ? "bg-amber-50 text-amber-600 border border-amber-200" : "bg-green-50 text-green-600 border border-green-200"}`}
              >
                {loading ? "..." : (selectedTutor.is_onboarded ? "Deactivate Tutor" : "Activate Tutor")}
              </button>
              <button 
                onClick={handleClose}
