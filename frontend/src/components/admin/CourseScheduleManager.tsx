...
export default function CourseScheduleManager({ courses: initialCourses }: { courses: any[] }) {
  ...
  const handleUpdateSchedule = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      if (day && time) {
        newSchedules.push({ day_of_week: day, start_time: `${time}:00`, duration_minutes: 60 });
      }
    }
    ...
    try {
      const res = await updateCourseSchedules(editingCourse.id, formData);
      if (res.error) {
        // Revert optimistic update on error
        setCourses(prev => prev.map(c =>
          c.id === editingCourse.id ? { ...c, schedules: editingCourse.schedules } : c
        ));
      ...
      if (res.data?.schedules) {
        setCourses(prev => prev.map(c =>
          c.id === editingCourse.id ? { ...c, schedules: res.data.schedules } : c
        ));
      }
    } finally {
      setSaving(false);
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-section-alt/50 border-b border-border">
                <th className="px-5 py-3 text-[11px] font-bold uppercase tracking-widest text-muted">Course Info</th>
                <th className="px-5 py-3 text-[11px] font-bold uppercase tracking-widest text-muted">Weekly Schedule (Max 3)</th>
            ...
            <tbody className="divide-y divide-border">
                ...
                <tr key={c.id} className="hover:bg-section/5 transition-colors">
                  <td className="px-5 py-4">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] px-1.5 py-0.5 bg-section-alt rounded text-muted font-medium leading-none">{c.category}</span>
                      {c.tutor_name && <span className="text-[10px] text-muted-foreground italic">with {c.tutor_name}</span>}
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    {c.schedules && c.schedules.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {c.schedules.map((s: any, idx: number) => (
                          <div key={idx} className="flex items-center gap-1.5 bg-primary/5 border border-primary/10 px-2 py-1 rounded-md text-[10px] font-bold text-primary">
                            <Calendar size={10} />
                            <span className="capitalize">{s.day_of_week}</span>
                            <Clock size={10} className="ml-1 opacity-60" />
                            {typeof s.start_time === 'string' ? s.start_time.slice(0, 5) : s.start_time}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <span className="text-xs text-muted italic">No schedule set.</span>
                    )}
                  </td>
              ...
              {filteredCourses.length === 0 && (
                <tr>
                  <td colSpan={3} className="p-12 text-center text-muted text-sm italic">No courses found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card w-full max-w-2xl rounded-2xl shadow-2xl border border-border overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-6 border-b border-border bg-section-alt/50">
              <div>
                <h3 className="text-lg font-bold" style={{ fontFamily: "'Lora', serif" }}>Edit Timetable</h3>
            ...
            <form onSubmit={handleUpdateSchedule} className="p-6 overflow-y-auto space-y-6">
              <div className="space-y-3">
                <h4 className="text-sm font-bold">Weekly Schedule (3 sessions per week)</h4>
                <p className="text-xs text-muted">Set the fixed timetable. Session dates will be generated automatically based on these rules.</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                  {[1, 2, 3].map(num => {
                    const sched = editingCourse.schedules?.[num - 1] || {};
                    const dayDefault = sched.day_of_week || '';
                    const rawTime = sched.start_time || '';
                    const timeDefault = rawTime.length >= 5 ? rawTime.slice(0, 5) : rawTime;
                    return (
                      <div key={num} className="p-3 bg-section-alt rounded-xl border border-border space-y-2">
